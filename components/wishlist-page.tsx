...
export function WishlistPage({ adminEmail }: WishlistPageProps) {
  ...
  const addWatchlistIgnToStore = useBonfireStore(
    (state) => state.addWatchlistIgn,
  );
  const removeWatchlistIgnFromStore = useBonfireStore(
    (state) => state.removeWatchlistIgn,
  );

  const matchedPlayers = records.filter((record) =>
    watchlistIgns.includes(normalizeIgn(record.ign)),
  );
      ...
      <section className="grid gap-6 xl:grid-cols-[0.8fr,1.2fr]">
        <div className="space-y-6">
          <Card className="border-border/80 bg-card/85">
            <CardHeader>
              ...
              <CardDescription>
                Save a trainer name here without going through the Search tab.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex gap-3">
                ...
                <Button
                  disabled={
                    !isValidIgn(newIgn) ||
                    watchlistIgns.includes(normalizedNewIgn)
                  }
                  onClick={async () => {
              ...
              <p className="text-xs text-muted-foreground">
                IGN must be 3-15 valid characters.
              </p>
              {message ? (
            ...
            <CardContent className="space-y-3">
              {watchlistIgns.length > 0 ? (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {watchlistMatches.includes(ign)
                          ? "Matched in latest sync"
                          : "Waiting for a match"}
                      </div>
                    </div>
                    <Button
                ...
                <div className="text-sm text-muted-foreground">
                  Your wishlist is empty. Add missing IGNs here or from the
                  Search tab.
                </div>
              )}
            </CardContent>
        ...
        <Card className="border-border/80 bg-card/85">
          <CardHeader>
            ...
            <CardDescription>
              Newly found trainers from the wishlist are highlighted here.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {matchedPlayers.length > 0 ? (
              ...
              <div className="text-sm text-muted-foreground">
                No wishlist matches yet. When a watched IGN appears after a
                fresh sync, it will show up here.
              </div>
            )}
          </CardContent>
        </Card>
