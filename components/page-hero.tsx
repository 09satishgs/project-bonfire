...
export function PageHero({ badge, title, description, aside }: PageHeroProps) {
  return (
    <section className="relative overflow-hidden rounded-[2rem] border border-border/80 bg-card/55 px-6 py-10 shadow-glow backdrop-blur md:px-10">
      ...
      <div className="grid gap-8 lg:grid-cols-[1.2fr,0.8fr]">
        <div className="space-y-5">
          <Badge className="bg-card/85 text-foreground">{badge}</Badge>
        ...
        {aside ? <Card className="border-border/80 bg-card/85"><CardContent className="p-6">{aside}</CardContent></Card> : null}
      </div>
    </section>
  );
}
