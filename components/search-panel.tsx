...
export function SearchPanel() {
  ...
  const alreadyWatching = missingQuery
    ? watchlistIgns.includes(missingQuery.trim().toLowerCase())
    : false;
  ...
  return (
    <Card className="border-border/80 bg-card/80 backdrop-blur">
      <CardHeader>
        ...
        <CardDescription>
          Filter by tags or IGN. You can use dropdown towards the end to order
          the data from A-Z or New-Old, etc...
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-[2fr,1fr,1fr]">
          ...
          <Select
            onChange={(event) =>
              setFilters({
                contactMethod: event.target
                  .value as typeof filters.contactMethod,
              })
            }
        ...
        {missingQuery ? (
          <div className="rounded-2xl border border-dashed border-border bg-secondary/60 p-4">
            <div className="text-sm font-medium">
              No match for "{missingQuery}" yet.
            </div>
            <p className="mt-1 text-sm text-muted-foreground">
              Save this IGN locally and we&apos;ll notify you after the next
              fresh sheet sync if they appear.
            </p>
                ...
                onClick={() =>
                  startTransition(() => {
                    void (async () => {
                      ...
                      setWatchMessage(
                        `${missingQuery} added to your watchlist.`,
                      );
                    })();
                  })
                }
